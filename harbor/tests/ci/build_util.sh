...
function publishImage {
    ...
    # for master, will use 'dev-arm' as the tag name
    # for release-*, will use 'release-*-dev-arm' as the tag name, like release-v1.8.0-dev-arm
    if [[ $1 == "master" ]]; then
      image_tag=dev=arm
    fi
    if [[ $1 == "release-"* ]]; then
    ...
    # rename the images with tag "dev-arm" and push to Docker Hub
    docker images
